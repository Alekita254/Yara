...
export const singleProductLoader =  async ({ params }) => {
  ...
  try {
    ...
    if (response.data) {
      return { productData: response.data }; // Return response.data instead of response
    } else {
...
const SingleProduct = () => {
  ...
  if (!productData) {
    return null; // Render nothing if productData is not available yet
  }
  ...
  const product = {
    ...
    isInWishList:
      wishItems.find((item) => item.id === productData.id) !== undefined,
  };
  ...
  const addToWishlistHandler = async (product) => {
    try {
      const getResponse = await axios.get(
        `http://localhost:8080/user/EDVYu6rC_G9SL9Z7-H9tr`
      );
  ...
  const handleSubmit = async (product, quantity) => {
    ...
    console.log ("The product received is: ", requestData);

    yaraRequestPost(url, requestData)
    .then((data) => {
